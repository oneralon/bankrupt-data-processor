...
options =
  ...
  follow_max: 100
  referer: 'https://www.fabrikant.ru/trades/procedure/search/?filter_id=6&count_on_page=40'
...
proceed = (number, etp, cb) ->
  Sync =>
    try
      ...
      console.log $('.Search-result-count').length
      ...
      $('.Search-result-item').each ->
        num = $(@).find('div.Search-item-option:contains("Аукцион продавца")').text().trim().match(/\d+/)[0]
