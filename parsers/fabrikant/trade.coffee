...
module.exports = (html, etp, url, ismicro, cb) ->
  ...
  Sync =>
    try
      ...
      trade.type = $('td.fname:contains("Форма аукциона")').next().text() or  $('td.fname:contains("Форма торга")').next().text()
      ...
      if $('td.fname:contains("Лоты")').next().find('table').text().length is 0
        ...
        lot.number = 1
        lot.payment_account = trade.debtor.payment_terms
        ...
        lot.information = $('td.fname:contains("реализуемого")').next().clone().children().remove().end().text().trim() + ': '
        lot.information += $('td.fname:contains("реализуемого")').next().find('b').text().trim() + '. '
        lot.information += 'Месторасположение предмета торгов: ' + $('td.fname:contains("Месторасположение предмета торгов:")').next().text().trim()
