    ...
    try
      ...
      bids.forEach (bid)->
        if bid.BidId?
          ...
          lot = {}
          lot.url = url
          lot.number = parseInt bid.BidNo
          lot.title = bid.BidName
          lot.status = bid.BidStatus
          lot.procedure = bid.BidParticipantDocuments
          lot.information = bid.BidName
          lot.tags = []
          lot.information
          lot.start_price = bid.BidPrice
          lot.step_percent = bid.AuctionStepMinPercent
          lot.deposit_procedure = bid.BidParticipantDemand
        trade.lots.push lot
      cb null, trade
    catch e then cb(e)
