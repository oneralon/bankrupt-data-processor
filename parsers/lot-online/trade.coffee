  ...
  needle.get url, options, (err, resp, body) ->
    ...
    needle.head 'http://bankruptcy.lot-online.ru/e-auction/accessDenied.xhtml', options, ->
      ...
      needle.head 'http://bankruptcy.lot-online.ru/e-auction/accessDenied.xhtml', options, ->
        ...
        trade.debtor.full_name = $('fieldset > legend:contains("Должник")').parent().find('div.form-item:contains("Наименование")').clone().children().remove().end().text().trim()
        trade.debtor.short_name = $('fieldset > legend:contains("Должник")').parent().find('div.form-item:contains("Краткое наименование")').clone().children().remove().end().text().trim()
