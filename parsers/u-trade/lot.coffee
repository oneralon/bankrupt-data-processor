...
module.exports = (html, etp, additional) ->
  ...
  if rows.length is 0 then rows = $("table")
  for row in rows
    lot = {}
    ...
    t2 = $(row).find("td:contains('Предмет торгов')")?.next().text().trim()
    lot.title = t0 or t1 or t2
    lot.number = $(row).find('span.dashed_underline')?.text().trim().match(/Лот №(\d+):.+/)?[1].trim() or '1'
    lot.information = $(row).find("td:contains('Cведения об имуществе (предприятии) должника, выставляемом на торги, его составе, характеристиках, описание')")?.next().text().trim()
    lot.reviewing_property = $(row).find("td:contains('Порядок ознакомления с имуществом (предприятием) должника')")?.next().text().trim()
    lot.start_price = parseFloat $(row).find("td:contains('Начальная цена продажи имущества')")?.next().text().trim().match(/([\d\s]+\,\d+)/)[0].replace(/\s/g, '')
    step = $(row).find("td:contains('Величина повышения начальной цены')")?.next().text().trim()
    lot.step_percent = parseFloat step.match(/(\d+\,\d+)%/)
    lot.step_sum = parseFloat step.match(/\(([\d\s]+\,\d+)\sруб\.\)/)?[1].replace(/\s/, '')
    lot.status = status $(row).find("td:contains('Статус торгов')")?.next().text().trim()
    deposit_size = $(row).find("td:contains('Размер задатка')")?.next().text().trim()
    ...
    lot.price_reduction_type = $(row).find("td:contains('Порядок снижения цены')")?.next().text().trim()
