...
module.exports = (html, etp, url, ismicro, cb) ->
  ...
  trade.holding_date = $("th:contains('Информация о торгах')").first().parent().parent().parent().find("td:contains('Дата и время подведения результатов торгов')").first().next().text().trim()
  trade.holding_date = moment(trade.holding_date, "DD.MM.YYYY HH:mm")
  trade.membership_type = $("th:contains('Информация о торгах')").first().parent().parent().parent().find("td:contains('Форма проведения торгов и подачи предложений')").first().next().text().trim()
  trade.submission_procedure = $("th:contains('Информация о торгах')").first().parent().parent().parent().find("td:contains('Порядок, место, срок и время представления заявок на участие в торгах и предложений о цене имущества (предприятия) должника')").first().next().text().trim()
  ...
  trade.requests_start_date = $("th:contains('Информация о торгах')").first().parent().parent().parent().find("td:contains('Начало предоставления заявок на участие')").first().next().text().trim()
  trade.requests_start_date = moment(trade.requests_start_date, "DD.MM.YYYY HH:mm")
  trade.requests_end_date = $("th:contains('Информация о торгах')").first().parent().parent().parent().find("td:contains('Окончание предоставления заявок на участие')").first().next().text().trim()
  trade.requests_end_date = moment(trade.requests_end_date, "DD.MM.YYYY HH:mm")
  trade.results_place = $("th:contains('Информация о торгах')").first().parent().parent().parent().find("td:contains('Место подведения результатов торгов')").first().next().text().trim()
  trade.additional = $("th:contains('Информация о торгах')").first().parent().parent().parent().find("td:contains('Порядок оформления участия в торгах, перечень представляемых участниками торгов документов и требования к их оформлению')").first().next().text().trim()
  trade.owner = {}
